#!/bin/sh
function help () {
echo "clip_ned2xyz.sh - A script that converts all img files in a directory to smaller chunks and then to xyz."
	echo "Usage: $0 chunk_dims resamp cellsize"
	echo "* chunk_dims: <number of rows/columns per chunk in resamp resolution>"
	echo "* resamp: <resample the img, yes or no>"
...
#get count of img files
total_files=$(ls -1 | grep '\.img$' | wc -l)
...
do
	...
	then
		...
		#exit 1
		gdalwarp $i -dstnodata -999999 -r cubicspline -tr $resamp_res $resamp_res -t_srs EPSG:4269 $(basename $i .img)".tif" -overwrite
		#mv $i $(basename $i .img)"_orig.img"
		#mv $(basename $i .img)"_resamp.tif" $i
	else
		echo -- Keeping orig resolution, but changing nodata value to -999999
		gdalwarp $i -dstnodata -999999 $(basename $i .img)".tif"
		#mv $i $(basename $i .img)"_orig.img"
		#mv $i"_tmp.tif" $(basename $i .img)".tif"
	...
	gdal_rasterize -burn -999999 $invert_param -l $shp_full $shp_full".shp" $(basename $i .img)".tif"

	#get input grid dimensions
	x_dim=`gdalinfo $(basename $i .img)".tif" | grep -e "Size is" | awk '{print $3}' | sed 's/.$//'`
	y_dim=`gdalinfo $(basename $i .img)".tif" | grep -e "Size is" | awk '{print $4}'`
	...
	while [ "$(bc <<< "$xoff < $x_dim")" == "1"  ]; do
	    ...
	    gdal_translate -of GTiff -srcwin $xoff $yoff $chunk_dim_x_int $chunk_dim_y_int $(basename $i .img)".tif" $chunk_name_full_clip -stats
	    valid_check=
